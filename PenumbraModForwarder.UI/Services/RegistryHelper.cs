...
public class RegistryHelper : IRegistryHelper
{
    ...
    public void CreateFileAssociation(IEnumerable<string> extensions, string applicationPath)
    {
        ...
        try
        {
            ...
            _logger.Information("Successfully created file associations for {Application}", applicationPath);
    ...
    public void RemoveFileAssociation(IEnumerable<string> extensions)
    {
        ...
        try
        {
            ...
            _logger.Information("File associations removed successfully.");
    ...
    {
        _logger.Debug("Entering AddApplicationToStartup with AppName={AppName}, AppPath={AppPath}", appName, appPath);

        if (string.IsNullOrWhiteSpace(appName) || string.IsNullOrWhiteSpace(appPath))
        {
            _logger.Error("Invalid arguments for adding startup registry key. AppName: {AppName}, AppPath: {AppPath}", appName, appPath);
            return;
        }
        ...
        try
        {
            ...
            _logger.Information("Application {AppName} added to startup at {AppPath}", appName, appPath);
        }
        catch (Exception e)
        {
    ...
    public void RemoveApplicationFromStartup(string appName)
    {
        ...
        try
        {
            ...
            _logger.Information("Application {AppName} removed from startup.", appName);
        }
        catch (Exception e)
        {
